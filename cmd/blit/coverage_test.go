...
func TestLatestCoverageFile(t *testing.T) {
	// When neither .blit/coverage.out nor any .out file exists, returns "".
	...
	// Create a .blit/coverage.out and verify it is found.
	if err := os.MkdirAll(".blit", 0o755); err != nil {
		t.Fatal(err)
	}
	outPath := filepath.Join(".blit", "coverage.out")
