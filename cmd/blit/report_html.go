...
func cmdReport(outPath string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[blit report] error loading history: %v\n", err)
		return 1
	}
	if len(records) == 0 {
		fmt.Fprintln(os.Stderr, "[blit report] no history found — run some tests first")
		return 1
	}
	...
	if err := os.WriteFile(outPath, []byte(content), 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "[blit report] write failed: %v\n", err)
		return 1
	}
	fmt.Printf("[blit report] wrote %s (%d runs)\n", outPath, len(records))
	return 0
}
