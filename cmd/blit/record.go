...
// record.go implements the `blit record <name> -- <command>` subcommand.
//
...
// recordStep mirrors btest.SessionStep but lives in cmd/blit to avoid
...
// runRecord is the entry point for `blit record`.
func runRecord(args []string) int {
	fs := flag.NewFlagSet("record", flag.ContinueOnError)
	outDir := fs.String("dir", "testdata/sessions", "directory to write .tuisess files")
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "usage: blit record [flags] <name> -- <command> [args...]")
		fs.PrintDefaults()
	}

	// Split on "--" to separate blit flags from the target command.
	splitIdx := -1
	for i, a := range args {
		if a == "--" {
			splitIdx = i
	...
	if splitIdx < 0 {
		fmt.Fprintln(os.Stderr, "blit record: missing '--' separator before command")
		fs.Usage()
		return 1
	}

	blitArgs := args[:splitIdx]
	cmdArgs := args[splitIdx+1:]

	if err := fs.Parse(blitArgs); err != nil {
		return 1
	}
	if fs.NArg() < 1 {
		fmt.Fprintln(os.Stderr, "blit record: <name> required")
		fs.Usage()
		return 1
	}
	if len(cmdArgs) == 0 {
		fmt.Fprintln(os.Stderr, "blit record: command required after '--'")
		fs.Usage()
		return 1
	}
...
func runRecordSession(sess *recordSession, outDir string) int {
	...
	fmt.Fprintf(os.Stderr, "[blit] recording %q → %s/%s.tuisess\n", sess.Name, outDir, sess.Name)
	fmt.Fprintln(os.Stderr, "[blit] ctrl+p = pause/resume  ctrl+c = stop+save")
	...
	if startErr := cmd.Start(); startErr != nil {
		...
		fmt.Fprintf(os.Stderr, "[blit] failed to start command: %v\n", startErr)
		return 1
	}
...
func saveRecordedSession(sess *recordSession, outDir string, cols, lines int) int {
	if err := os.MkdirAll(outDir, 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "[blit] mkdir %s: %v\n", outDir, err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[blit] marshal: %v\n", err)
		return 1
	}
	...
	if err := os.WriteFile(tmp, data, 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "[blit] write tmp: %v\n", err)
		return 1
	}
	if err := os.Rename(tmp, dest); err != nil {
		fmt.Fprintf(os.Stderr, "[blit] rename: %v\n", err)
		return 1
	}

	elapsed := time.Since(sess.StartedAt).Round(time.Second)
	fmt.Fprintf(os.Stderr, "[blit] saved %s (%d keystrokes, %s)\n", dest, sess.keystroke, elapsed)
	return 0
}
