// review.go implements `blit review` — an interactive TUI that presents
...
type reviewModel struct {
	items     []btest.PendingGolden
	cursor    int
	width     int
	height    int
	viewer    *btest.DiffViewer
	done      bool
	statusMsg string
}

func newReviewModel(items []btest.PendingGolden) reviewModel {
...
func buildViewer(pg btest.PendingGolden) *btest.DiffViewer {
	fc := &btest.FailureCapture{
...
func (m reviewModel) View() string {
	...
	if m.done || total == 0 {
		b.WriteString(reviewHeaderStyle.Render(" blit review ") + "\n\n")
	...
	// Header
	header := fmt.Sprintf(" blit review  %d/%d pending ", m.cursor+1, total)
	if m.width > 0 {
...
// runReview implements the `blit review` subcommand. It scans the given
...
func runReview(args []string) int {
	...
	items, err := btest.FindPendingGoldens(root)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[blit review] scan error: %v\n", err)
		return 1
	}
	if len(items) == 0 {
		fmt.Println("[blit review] no pending snapshots found")
		return 0
	}
	...
	if _, err := p.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "[blit review] TUI error: %v\n", err)
		return 1
	}
	return 0
}
