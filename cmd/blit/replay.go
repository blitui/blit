...
// replay.go implements the `blit replay <name>` subcommand.
//
...
// runReplay is the entry point for `blit replay`.
func runReplay(args []string) int {
	...
	if err != nil {
		...
		if filepath.Ext(name) == "" {
			fmt.Fprintf(os.Stderr, "[blit] cannot read %s: %v\n", path, err)
			return 1
		}
	...
	if err := json.Unmarshal(data, &sess); err != nil {
		fmt.Fprintf(os.Stderr, "[blit] parse %s: %v\n", path, err)
		return 1
	}
	if sess.Version != 1 && sess.Version != 2 {
		fmt.Fprintf(os.Stderr, "[blit] unsupported session version %d\n", sess.Version)
		return 1
	}
...
func runReplaySession(sess *replaySessFile, path string, speed float64, skipKeys map[string]bool) int {
	...
	if total == 0 {
		fmt.Fprintf(os.Stderr, "[blit] session %s has no steps\n", path)
		return 0
	}
	...
	fmt.Fprintf(os.Stderr, "[blit] replaying %q (%d steps, %.1fx speed)\n",
		sessionName, total, speed)
	if len(sess.Command) > 0 {
		fmt.Fprintf(os.Stderr, "[blit] original command: %s\n", strings.Join(sess.Command, " "))
	}
	...
	fmt.Fprintf(os.Stderr, "[blit] replay complete in %s\n", elapsed)
...
// the full blit package (which is testing-only).
