...
func writeTempGolden(t *testing.T, dir, name, expected, actual string) btest.PendingGolden {
	t.Helper()
...
func TestReviewFindPendingGoldens(t *testing.T) {
	...
	items, err := btest.FindPendingGoldens(dir)
	if err != nil {
...
func TestReviewModelKeyBindings(t *testing.T) {
	...
	items := []btest.PendingGolden{pg1, pg2}
...
func TestReviewPendingGoldenTestName(t *testing.T) {
	pg := btest.PendingGolden{GoldenPath: filepath.Join("testdata", "login-form.golden")}
	want := "login-form"
