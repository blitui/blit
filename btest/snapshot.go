...
// updateSnapshots is set by the -btest.update flag. When true,
...
var updateSnapshots = flag.Bool("btest.update", false,
	"regenerate btest screen snapshots instead of comparing")
...
// run (or when -btest.update is passed), the snapshot is (re)generated.
...
func AssertSnapshot(t testing.TB, scr *Screen, name string) {
	...
	if *updateSnapshots {
		...
		t.Logf("btest: wrote snapshot %s", path)
		return
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			t.Logf("btest: created snapshot %s (first run)", path)
			return
		}
	...
	if got != wantStr {
		t.Errorf("snapshot %q mismatch\n--- want ---\n%s\n--- got ---\n%s\n"+
			"(rerun with -args -btest.update to regenerate)",
			name, wantStr, got)
	}
}
