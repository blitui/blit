...
// DiffViewer is a blit Component that renders a side-by-side (or unified /
// cells-only) comparison of two screens captured from a failing btest
// assertion. It implements the blit Component interface so it can be
...
// Update implements blit.Component. ctx is the ambient blit.Context but
// the DiffViewer only needs key messages, so it accepts tea.Msg directly and
// ignores the ctx value (kept as interface{} to avoid importing blit from
// inside the btest sub-package, which would create a cycle).
